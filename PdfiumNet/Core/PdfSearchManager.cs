...
namespace PdfiumViewer.Core
{
	...
	public class PdfSearchManager
	{
		...
		public int CurrentMatchIndex => _offset;
		...
		public bool Search(string text)
		{
			...
			if (Renderer.Document != null && !string.IsNullOrEmpty(text))
			{
				...
				// Fixed sorting: sort by page first, then by Y position (top to bottom)
				//_bounds = _bounds.OrderBy(b => b[0].Page).ThenBy(b => b[0].Bounds.Y).ToList();
		...
		private List<IList<PdfRectangle>> GetAllBounds(PdfMatches matches)
		{
		...
		public bool FindNext(bool forward, out int page)
		{
			page = 1; // Default page
			...
			if (_offset == -1)
			{
				// First search - start from beginning
				_offset = 0;
				_firstMatch = _offset;
			}
			else
			{
				...
				if (forward)
				{
					...
					if (_offset >= _bounds.Count)
						_offset = 0; // Wrap to beginning
				}
				else
				{
					_offset--;
					if (_offset < 0)
						_offset = _bounds.Count - 1; // Wrap to end
				}
			}
			...
			page = current[0].Page + 1; // Convert to 1-based page number
			return true;
		}

		/// <summary>
		/// Goes to the first match
		/// </summary>
		public bool GoToFirstMatch(out int page)
		{
			page = 1;
			...
			var current = _bounds[_offset];
			page = current[0].Page + 1;
			return true;
		}

		private void ScrollCurrentIntoView()
		{
			if (_offset == -1 || _bounds == null || _offset >= _bounds.Count)
				return;
		...
		{
			if (index < 0 || index >= _bounds.Count) return;
